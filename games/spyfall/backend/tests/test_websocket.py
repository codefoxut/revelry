...
def test_start_game_with_enough_players_broadcasts_discussion_phase(isolated_manager):
    room, host_id = _create_room(isolated_manager)
    for i in range(3):
    ...
    with client.websocket_connect(f"/ws/{room.code}?player_id={host_id}") as host_socket:
        ...
        with client.websocket_connect(f"/ws/{room.code}?player_id={guest_ids[0]}") as g0, \
            ...
            for socket in (host_socket, g0, g1, g2):
                ...
                socket.receive_json()  # discussion_timer_started broadcast

    assert all(event["type"] == "role_assigned" for event in role_events)
    spy_count = sum(1 for event in role_events if event["is_spy"])
    assert spy_count == 1
...
def test_advance_phase_moves_discussion_to_voting_and_broadcasts(isolated_manager):
    room, host_id = _create_room(isolated_manager)
    for i in range(3):
        _join_room(isolated_manager, room.code, display_name=f"Player{i}")
    client = TestClient(app)
    ...
    assert update["type"] == "room_state"
    game_state = update["room"]["game_state"]
    assert game_state["phase"] == "voting"
    assert game_state["round_number"] == 1
...
def test_reconnect_after_start_resends_own_assignment(isolated_manager):
    ...
    assert reconnect_role_event["is_spy"] == first_role_event["is_spy"]
    assert reconnect_role_event["role"] == first_role_event["role"]
