...
import { LOCATION_CATALOG } from "@/lib/locations";

// Mirrors MIN_PLAYERS in app/games/spyfall/__init__.py.
const MIN_PLAYERS_TO_START = 3;

const ALL_LOCATION_KEYS = LOCATION_CATALOG.map((location) => location.key);
...
export function LobbyView() {
  ...
  const [enabledLocationKeys, setEnabledLocationKeys] = useState<string[]>(ALL_LOCATION_KEYS);
  ...
  function toggleLocation(key: string) {
    setEnabledLocationKeys((current) =>
      current.includes(key) ? current.filter((k) => k !== key) : [...current, key],
    );
  }

  function selectAllLocations() {
    setEnabledLocationKeys(ALL_LOCATION_KEYS);
  }

  function selectNoLocations() {
    setEnabledLocationKeys([]);
  }

  function startGame() {
    // The backend rejects an empty enabled_location_keys list, so the full
    // bank is represented by omitting the field entirely rather than
    // sending every key explicitly.
    const allSelected = enabledLocationKeys.length === ALL_LOCATION_KEYS.length;
    sendCommand({
      type: "start_game",
      ...(allSelected ? {} : { enabled_location_keys: enabledLocationKeys }),
    });
  }
      ...
      <div className="flex w-full max-w-lg flex-col gap-6">
        ...
        {!inGame && isHost && (
          <div className="flex flex-col items-center gap-4">
            <div className="flex w-full flex-col gap-3 rounded-xl border border-zinc-800 bg-zinc-900 p-4">
              <div className="flex items-center justify-between gap-3">
                <span className="text-xs uppercase tracking-wide text-zinc-500">
                  Locations ({enabledLocationKeys.length}/{ALL_LOCATION_KEYS.length})
                </span>
                <div className="flex gap-3 text-xs">
                  <button
                    type="button"
                    onClick={selectAllLocations}
                    className="text-zinc-400 underline decoration-dotted underline-offset-4 hover:text-zinc-200"
                  >
                    Select all
                  </button>
                  <button
                    type="button"
                    onClick={selectNoLocations}
                    className="text-zinc-400 underline decoration-dotted underline-offset-4 hover:text-zinc-200"
                  >
                    Select none
                  </button>
              ...
              <div className="grid max-h-64 grid-cols-2 gap-1 overflow-y-auto sm:grid-cols-3">
                {LOCATION_CATALOG.map((location) => {
                  const enabled = enabledLocationKeys.includes(location.key);
                  return (
                    <button
                      key={location.key}
                      type="button"
                      onClick={() => toggleLocation(location.key)}
                      className={`rounded-lg border px-2 py-1.5 text-left text-sm transition-colors ${
                        enabled
              ...
              {enabledLocationKeys.length === 0 && (
                <p className="text-center text-sm text-rose-400">
                  Select at least one location to start.
                </p>
              )}
            </div>
            ...
            <button
              type="button"
              onClick={startGame}
              disabled={activePlayerCount < MIN_PLAYERS_TO_START || enabledLocationKeys.length === 0}
