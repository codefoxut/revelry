...
from app.websocket.dispatcher import dispatch_client_event, send_psychic_secret
...
async def room_socket(
    ...
    # Resend psychic secret on reconnect — get_target_position returns None for
    # non-Psychics and during REVEAL (when target is already in room_state), so
    # this is a no-op for everyone except the active Psychic mid-round.
    await send_psychic_secret(room_code, player_id, games, connections)
