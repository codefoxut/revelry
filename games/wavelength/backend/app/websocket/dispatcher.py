...
from app.games.wavelength.events import ClueGivenEvent as EngineClueGivenEvent
from app.games.wavelength.events import GameOverEvent as EngineGameOverEvent
from app.games.wavelength.events import PlayerGuessedEvent as EnginePlayerGuessedEvent
from app.games.wavelength.events import RevealedEvent as EngineRevealedEvent
from app.games.wavelength.events import RoundStartedEvent as EngineRoundStartedEvent
...
async def dispatch_client_event(
    ...
    *,
    ...
    if event_type == "submit_guess":
        await _handle_submit_guess(raw_event, room_code, player_id, room_manager, game_session_manager, connection_manager)
        return False
...
async def _handle_start_game(
    ...
    await broadcast_room_state(room_code, room_manager, connection_manager, game_session_manager)
    # Send target position privately to the Psychic
    if events and isinstance(events[0], EngineRoundStartedEvent):
        psychic_id = events[0].psychic_id
        await send_psychic_secret(room_code, psychic_id, game_session_manager, connection_manager)
...
async def _handle_give_clue(
    ...
    clue_text = raw_event.get("clue_text", "")
    if not isinstance(clue_text, str):
        await _send_error(connection_manager, room_code, player_id, "invalid_payload", "`clue_text` must be a string")
        return

    try:
        events = await game_session_manager.give_clue(room_code, player_id, clue_text)
...
async def _handle_submit_guess(
    ...
    if not isinstance(position, (int, float)) or isinstance(position, bool):
        await _send_error(connection_manager, room_code, player_id, "invalid_payload", "`position` must be a number")
        return

    try:
        events = await game_session_manager.submit_guess(room_code, player_id, float(position))
    except GameNotStartedError as exc:
        await _send_error(connection_manager, room_code, player_id, "game_not_started", str(exc))
        return
    except PermissionDeniedError as exc:
...
async def _handle_next_round(
    ...
    await _broadcast_game_events(events, room_code, connection_manager)
    await broadcast_room_state(room_code, room_manager, connection_manager, game_session_manager)
    # If a new round started, send the target privately to the new Psychic
    if events and isinstance(events[0], EngineRoundStartedEvent):
        psychic_id = events[0].psychic_id
        await send_psychic_secret(room_code, psychic_id, game_session_manager, connection_manager)
    ...
    for event in events:
        if isinstance(event, EngineClueGivenEvent):
            await connection_manager.broadcast(
                room_code, ClueGivenEvent(clue_text=event.clue_text, psychic_id=event.psychic_id)
            )
        elif isinstance(event, EnginePlayerGuessedEvent):
            await connection_manager.broadcast(
                room_code, PlayerGuessedEvent(player_id=event.player_id, guessed_count=event.guessed_count)
            )
...
async def send_psychic_secret(
    room_code: str,
    player_id: str,
    game_session_manager: GameSessionManager,
    connection_manager: ConnectionManager,
) -> None:
    """Send the target position privately to the Psychic.

    Called right after round start and on reconnect (for all players, but
    get_target_position() returns None for non-Psychics so only the Psychic
    actually receives the event).
    """
    target = await game_session_manager.get_target_position(room_code, player_id)
    if target is not None:
        await connection_manager.send_to_player(room_code, player_id, PsychicTargetEvent(target_position=target))
