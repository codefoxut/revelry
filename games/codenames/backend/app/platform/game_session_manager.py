...
from app.games.spyfall import DISPLAY_NAME, MIN_PLAYERS
...
class GameSessionManager:
    ...
    (room lifecycle) and SpyfallGameEngine (phase/rules). Spyfall-specific:
    ...
    async def start_game(
        ...
        engine = SpyfallGameEngine(room_code)
        events = await engine.handle_command(
            StartGameCommand(
                player_id=requester_id,
                active_player_ids=active_player_ids,
                enabled_location_keys=enabled_location_keys,
            )
        )
    ...
    async def guess_location(self, room_code: str, player_id: str, location_key: str) -> list[Event]:
        engine = await self._engine_store.get(room_code)
        if engine is None:
            raise GameNotStartedError(f"Room {room_code!r} hasn't started a game")

        return await engine.handle_command(GuessLocationCommand(player_id=player_id, location_key=location_key))
    ...
    async def get_assignment(self, room_code: str, player_id: str) -> tuple[bool, str | None, str | None] | None:
        """Pragmatic Spyfall-specific accessor, mirroring Mafia's
        `get_role()`: "what's this player's assignment" doesn't generalize
        across arbitrary future games the way phase_snapshot does, so this
        reaches into SpyfallGameEngine directly rather than adding a
        speculative generic method to GameEngine.
        """
        engine = await self._engine_store.get(room_code)
        if not isinstance(engine, SpyfallGameEngine):
            return None
        try:
            return engine.get_assignment(player_id)
