...
class RoomManager:
    ...
    async def create_room(
        ...
        room = Room(code=code, game_type=game_type, host_player_id=host_id, is_private=is_private, max_players=_MAX_PLAYERS)
    ...
    async def join_room(self, code: str, display_name: str, avatar: str = "default", as_spectator: bool = False) -> tuple[Room, str]:
        ...
        room.players[player_id] = Player(id=player_id, display_name=display_name, avatar=avatar, is_spectator=join_as_spectator)
        await self._room_store.save(room)
    ...
    async def update_profile(self, code: str, player_id: str, display_name: str | None = None, avatar: str | None = None) -> Room:
        room = await self.require_room(code)
        player = self._require_player(room, player_id)
