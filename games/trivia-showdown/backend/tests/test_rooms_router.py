...
async def test_create_room_returns_host_player_id():
    async with await _client() as client:
        response = await client.post(
            "/api/rooms",
            json={"game_type": "trivia_showdown", "host_display_name": "Alice"},
        )

    assert response.status_code == 201
    body = response.json()
    assert body["room"]["game_type"] == "trivia_showdown"
...
async def test_create_and_join_persist_avatar():
    async with await _client() as client:
        created = await client.post(
            "/api/rooms",
            json={"game_type": "trivia_showdown", "host_display_name": "Alice", "host_avatar": "fox"},
        )
        code = created.json()["room"]["code"]
